...
def count_neighbours(grid, cube):
    ...
    for offset in product([-1, 0, 1], repeat=4):
        if offset != (0, 0, 0, 0):
            if (cube[0] + offset[0], cube[1] + offset[1], cube[2] + offset[2], cube[3] + offset[3]) in grid:
                count += 1
    return count
...
def count_active(grid, cycles):
    ...
    for tick in range(cycles):
        ...
        for cube in product(range(gridstart - 1, gridsize + 2), repeat=4):
