...
class Packet:
    def __init__(self, version, type_id, value=None, contents=None):
...
def add_versions(inpacket):
    if not inpacket.contents:
        return inpacket.version
    else:
        subsum = 0
        for packet in inpacket.contents:
            subsum = add_versions(packet)
        return inpacket.version + subsum

def find_versioncount(instr):
...
def main():
    #assert find_versioncount('8A004A801A8002F478') == 16
    print( find_versioncount('620080001611562C8802118E34') )
    #assert find_versioncount('C0015000016115A2E0802F182340') == 23
    #assert find_versioncount('A0016C880162017C3686B18A3D4780') == 31
